...
def calculate_detection_rate(text1_path, text2_path, output_path):
    ...
    with open(output_path, 'w', encoding='utf-8') as out_file:
        ...
        with open("l_w_detection_rate_detail.txt", 'w', encoding='utf-8') as detail_file:
            ...
            total_wrong_word_ref_count = 0
            total_laughter_hyp_count = 0
            total_wrong_word_hyp_count = 0
            total_laughter_correct_count = 0
            total_wrong_word_correct_count = 0
            total_laughter_correct_count_relaxed = 0
            total_wrong_word_correct_count_relaxed = 0
            total_laughter_deletion_count = 0
            total_wrong_word_deletion_count = 0
            total_laughter_insertion_count = 0
            total_wrong_word_insertion_count = 0
            total_laughter_deletion_count_relaxed = 0
            total_wrong_word_deletion_count_relaxed = 0
            total_laughter_insertion_count_relaxed = 0
            total_wrong_word_insertion_count_relaxed = 0
            ...
            for i in range(1, len(sentences1), 2):
                ...
                wrong_word_ref_words = re.findall(wrong_word_pattern, sentence1)
                laughter_hyp_words = re.findall(laughter_pattern, sentence2)
                wrong_word_hyp_words = re.findall(wrong_word_pattern, sentence2)

                # DPを使ってアライメントを取得
                laughter_alignment = align_words(laughter_ref_words, laughter_hyp_words)
                wrong_word_alignment = align_words(wrong_word_ref_words, wrong_word_hyp_words)

                # 正解数、脱落誤り数、挿入誤り数を計算
                laughter_ref_count = len(laughter_ref_words)
                wrong_word_ref_count = len(wrong_word_ref_words)
                laughter_hyp_count = len(laughter_hyp_words)
                wrong_word_hyp_count = len(wrong_word_hyp_words)
                laughter_correct_count = sum(1 for ref, hyp in laughter_alignment if ref == hyp)
                wrong_word_correct_count = sum(1 for ref, hyp in wrong_word_alignment if ref == hyp)
                laughter_correct_count_relaxed = sum(1 for ref, hyp in laughter_alignment if ref and hyp)
                wrong_word_correct_count_relaxed = sum(1 for ref, hyp in wrong_word_alignment if ref and hyp)
                laughter_deletion_count = laughter_ref_count - laughter_correct_count
                wrong_word_deletion_count = wrong_word_ref_count - wrong_word_correct_count
                laughter_insertion_count = laughter_hyp_count - laughter_correct_count
                wrong_word_insertion_count = wrong_word_hyp_count - wrong_word_correct_count
                laughter_deletion_count_relaxed = laughter_ref_count - laughter_correct_count_relaxed
                wrong_word_deletion_count_relaxed = wrong_word_ref_count - wrong_word_correct_count_relaxed
                laughter_insertion_count_relaxed = laughter_hyp_count - laughter_correct_count_relaxed
                wrong_word_insertion_count_relaxed = wrong_word_hyp_count - wrong_word_correct_count_relaxed

                # Precision、Recall、F1スコアを計算
                laughter_precision = laughter_correct_count / laughter_hyp_count * 100 if laughter_hyp_count > 0 else 0
                wrong_word_precision = wrong_word_correct_count / wrong_word_hyp_count * 100 if wrong_word_hyp_count > 0 else 0
                laughter_recall = laughter_correct_count / laughter_ref_count * 100 if laughter_ref_count > 0 else 0
                wrong_word_recall = wrong_word_correct_count / wrong_word_ref_count * 100 if wrong_word_ref_count > 0 else 0
                laughter_precision_relaxed = laughter_correct_count_relaxed / laughter_hyp_count * 100 if laughter_hyp_count > 0 else 0
                wrong_word_precision_relaxed = wrong_word_correct_count_relaxed / wrong_word_hyp_count * 100 if wrong_word_hyp_count > 0 else 0
                laughter_recall_relaxed = laughter_correct_count_relaxed / laughter_ref_count * 100 if laughter_ref_count > 0 else 0
                wrong_word_recall_relaxed = wrong_word_correct_count_relaxed / wrong_word_ref_count * 100 if wrong_word_ref_count > 0 else 0
                ...
                wrong_word_f1 = 2 * wrong_word_precision * wrong_word_recall / (wrong_word_precision + wrong_word_recall) if wrong_word_precision + wrong_word_recall > 0 else 0
                ...
                wrong_word_f1_relaxed = 2 * wrong_word_precision_relaxed * wrong_word_recall_relaxed / (wrong_word_precision_relaxed + wrong_word_recall_relaxed) if wrong_word_precision_relaxed + wrong_word_recall_relaxed > 0 else 0
                ...
                out_file.write(f"言い誤りの検出率 - Precision: {wrong_word_precision:.2f}%, Recall: {wrong_word_recall:.2f}%, F1: {wrong_word_f1:.2f}%\n")
                ...
                out_file.write(f"言い誤りの検出率 (緩和) - Precision: {wrong_word_precision_relaxed:.2f}%, Recall: {wrong_word_recall_relaxed:.2f}%, F1: {wrong_word_f1_relaxed:.2f}%\n")
                out_file.write(f"笑いの脱落誤り数: {laughter_deletion_count}, 挿入誤り数: {laughter_insertion_count}\n")
                out_file.write(f"言い誤りの脱落誤り数: {wrong_word_deletion_count}, 挿入誤り数: {wrong_word_insertion_count}\n")
                out_file.write(f"笑いの脱落誤り数 (緩和): {laughter_deletion_count_relaxed}, 挿入誤り数 (緩和): {laughter_insertion_count_relaxed}\n")
                out_file.write(f"言い誤りの脱落誤り数 (緩和): {wrong_word_deletion_count_relaxed}, 挿入誤り数 (緩和): {wrong_word_insertion_count_relaxed}\n")
                out_file.write("\n")
                ...
                for ref, hyp in wrong_word_alignment:
                    if ref == hyp:
                        detail_file.write(f"  {ref} -- {hyp} (完全一致)\n")
                    elif ref and hyp:
                        detail_file.write(f"  {ref} -- {hyp} (部分一致)\n")
                ...
                total_wrong_word_ref_count += wrong_word_ref_count
                total_laughter_hyp_count += laughter_hyp_count
                total_wrong_word_hyp_count += wrong_word_hyp_count
                total_laughter_correct_count += laughter_correct_count
                total_wrong_word_correct_count += wrong_word_correct_count
                total_laughter_correct_count_relaxed += laughter_correct_count_relaxed
                total_wrong_word_correct_count_relaxed += wrong_word_correct_count_relaxed
                total_laughter_deletion_count += laughter_deletion_count
                total_wrong_word_deletion_count += wrong_word_deletion_count
                total_laughter_insertion_count += laughter_insertion_count
                total_wrong_word_insertion_count += wrong_word_insertion_count
                total_laughter_deletion_count_relaxed += laughter_deletion_count_relaxed
                total_wrong_word_deletion_count_relaxed += wrong_word_deletion_count_relaxed
                total_laughter_insertion_count_relaxed += laughter_insertion_count_relaxed
                total_wrong_word_insertion_count_relaxed += wrong_word_insertion_count_relaxed
            ...
            total_wrong_word_precision = total_wrong_word_correct_count / total_wrong_word_hyp_count * 100 if total_wrong_word_hyp_count > 0 else 0
            total_laughter_recall = total_laughter_correct_count / total_laughter_ref_count * 100 if total_laughter_ref_count > 0 else 0
            total_wrong_word_recall = total_wrong_word_correct_count / total_wrong_word_ref_count * 100 if total_wrong_word_ref_count > 0 else 0
            ...
            total_wrong_word_precision_relaxed = total_wrong_word_correct_count_relaxed / total_wrong_word_hyp_count * 100 if total_wrong_word_hyp_count > 0 else 0
            ...
            total_wrong_word_recall_relaxed = total_wrong_word_correct_count_relaxed / total_wrong_word_ref_count * 100 if total_wrong_word_ref_count > 0 else 0
            ...
            total_wrong_word_f1 = 2 * total_wrong_word_precision * total_wrong_word_recall / (total_wrong_word_precision + total_wrong_word_recall) if total_wrong_word_precision + total_wrong_word_recall > 0 else 0
            ...
            total_wrong_word_f1_relaxed = 2 * total_wrong_word_precision_relaxed * total_wrong_word_recall_relaxed / (total_wrong_word_precision_relaxed + total_wrong_word_recall_relaxed) if total_wrong_word_precision_relaxed + total_wrong_word_recall_relaxed > 0 else 0

            # テキスト全体の検出率を出力ファイルに書き込む
            out_file.write("テキスト全体での検出率:\n")
            out_file.write(f"笑いの総数: {total_laughter_ref_count}, 言い誤りの総数: {total_wrong_word_ref_count}\n")
            out_file.write(f"笑いの検出率 - Precision: {total_laughter_precision:.2f}%, Recall: {total_laughter_recall:.2f}%, F1: {total_laughter_f1:.2f}%\n")
            out_file.write(f"言い誤りの検出率 - Precision: {total_wrong_word_precision:.2f}%, Recall: {total_wrong_word_recall:.2f}%, F1: {total_wrong_word_f1:.2f}%\n")
            ...
            out_file.write(f"言い誤りの検出率 (緩和) - Precision: {total_wrong_word_precision_relaxed:.2f}%, Recall: {total_wrong_word_recall_relaxed:.2f}%, F1: {total_wrong_word_f1_relaxed:.2f}%\n")
            out_file.write(f"笑いの脱落誤り数: {total_laughter_deletion_count}, 挿入誤り数: {total_laughter_insertion_count}\n")
            out_file.write(f"言い誤りの脱落誤り数: {total_wrong_word_deletion_count}, 挿入誤り数: {total_wrong_word_insertion_count}\n")
            out_file.write(f"笑いの脱落誤り数 (緩和): {total_laughter_deletion_count_relaxed}, 挿入誤り数 (緩和): {total_laughter_insertion_count_relaxed}\n")
            out_file.write(f"言い誤りの脱落誤り数 (緩和): {total_wrong_word_deletion_count_relaxed}, 挿入誤り数 (緩和): {total_wrong_word_insertion_count_relaxed}\n")
