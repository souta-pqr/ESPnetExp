        ...
        with open("detection_rate_detail.txt", 'w', encoding='utf-8') as detail_file:
            ...
            for i in range(1, len(sentences1), 2):
                ...
                for ref, hyp in f_alignment:
                    ...
                    elif any(ref_word in hyp or hyp_word in ref for ref_word in ref.split() for hyp_word in hyp.split()) and abs(f_ref_words.index(ref) - f_hyp_words.index(hyp)) <= 1:
                ...
                for ref, hyp in d_alignment:
                    if ref == hyp:
                        detail_file.write(f"  {ref} -- {hyp} (完全一致)\n")
                    elif any(ref_word in hyp or hyp_word in ref for ref_word in ref.split() for hyp_word in hyp.split()) and abs(d_ref_words.index(ref) - d_hyp_words.index(hyp)) <= 1:
                        detail_file.write(f"  {ref} -- {hyp} (部分一致)\n")
                    elif ref:
                        detail_file.write(f"  {ref} -- (脱落)\n")
                    elif hyp:
