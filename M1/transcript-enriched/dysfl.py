...
def calculate_detection_rate(text1_path, text2_path, output_path):
    ...
    # dysflとカッコの間の文字列を抽出する正規表現パターン
    f_pattern = r'\<dysfl\>\s*([^(/]+)\s*\</dysfl\>'

    # 出力ファイルを開く
    with open(output_path, 'w', encoding='utf-8') as out_file:
        # 詳細出力用のファイルを開く
        with open("dysfl_detection_rate_detail.txt", 'w', encoding='utf-8') as detail_file:
            # 各文ごとにFの検出率を計算
            ...
            for i in range(1, len(sentences1), 2):
                ...
                out_file.write(f"非流暢性の検出率 - Precision: {f_precision:.2f}%, Recall: {f_recall:.2f}%, F1: {f_f1:.2f}%\n")
                out_file.write(f"非流暢性の検出率 (緩和) - Precision: {f_precision_relaxed:.2f}%, Recall: {f_recall_relaxed:.2f}%, F1: {f_f1_relaxed:.2f}%\n")
                out_file.write(f"非流暢性の脱落誤り数: {f_deletion_count}, 挿入誤り数: {f_insertion_count}\n")
                out_file.write(f"非流暢性の脱落誤り数 (緩和): {f_deletion_count_relaxed}, 挿入誤り数 (緩和): {f_insertion_count_relaxed}\n")
                out_file.write("\n")
            ...
            out_file.write(f"非流暢性の総数: {total_f_ref_count}\n")
            out_file.write(f"非流暢性の検出率 - Precision: {total_f_precision:.2f}%, Recall: {total_f_recall:.2f}%, F1: {total_f_f1:.2f}%\n")
            out_file.write(f"非流暢性の検出率 (緩和) - Precision: {total_f_precision_relaxed:.2f}%, Recall: {total_f_recall_relaxed:.2f}%, F1: {total_f_f1_relaxed:.2f}%\n")
            out_file.write(f"非流暢性の脱落誤り数: {total_f_deletion_count}, 挿入誤り数: {total_f_insertion_count}\n")
            out_file.write(f"非流暢性の脱落誤り数 (緩和): {total_f_deletion_count_relaxed}, 挿入誤り数 (緩和): {total_f_insertion_count_relaxed}\n")
...
text1_path = "REF/text"  # 正解文（REF）
text2_path = "HYP/text"  # 認識文（HYP）
output_path = "dysfl_detection_rate.txt"
