...
def calculate_detection_rate(text1_path, text2_path, output_path):
    ...
    with open(output_path, 'w', encoding='utf-8') as out_file:
        # 詳細出力用のファイルを開く
        with open("b_c_detection_rate_detail.txt", 'w', encoding='utf-8') as detail_file:
            # 各文ごとにFとDの検出率を計算
            ...
            for sentence1, sentence2 in zip(sentences1, sentences2):
                ...
                out_file.write(f"相槌の検出率 - Precision: {f_precision:.2f}%, Recall: {f_recall:.2f}%, F1: {f_f1:.2f}%\n")
                out_file.write(f"接続詞の検出率 - Precision: {d_precision:.2f}%, Recall: {d_recall:.2f}%, F1: {d_f1:.2f}%\n")
                out_file.write(f"相槌の検出率 (緩和) - Precision: {f_precision_relaxed:.2f}%, Recall: {f_recall_relaxed:.2f}%, F1: {f_f1_relaxed:.2f}%\n")
                out_file.write(f"接続詞の検出率 (緩和) - Precision: {d_precision_relaxed:.2f}%, Recall: {d_recall_relaxed:.2f}%, F1: {d_f1_relaxed:.2f}%\n")
                out_file.write(f"相槌の脱落誤り数: {f_deletion_count}, 挿入誤り数: {f_insertion_count}\n")
                out_file.write(f"接続詞の脱落誤り数: {d_deletion_count}, 挿入誤り数: {d_insertion_count}\n")
                out_file.write(f"相槌の脱落誤り数 (緩和): {f_deletion_count_relaxed}, 挿入誤り数 (緩和): {f_insertion_count_relaxed}\n")
                out_file.write(f"接続詞の脱落誤り数 (緩和): {d_deletion_count_relaxed}, 挿入誤り数 (緩和): {d_insertion_count_relaxed}\n")
                out_file.write("\n")
            ...
            out_file.write(f"相槌の総数: {total_f_ref_count}, 接続詞の総数: {total_d_ref_count}\n")
            out_file.write(f"相槌の検出率 - Precision: {total_f_precision:.2f}%, Recall: {total_f_recall:.2f}%, F1: {total_f_f1:.2f}%\n")
            out_file.write(f"接続詞の検出率 - Precision: {total_d_precision:.2f}%, Recall: {total_d_recall:.2f}%, F1: {total_d_f1:.2f}%\n")
            out_file.write(f"相槌の検出率 (緩和) - Precision: {total_f_precision_relaxed:.2f}%, Recall: {total_f_recall_relaxed:.2f}%, F1: {total_f_f1_relaxed:.2f}%\n")
            out_file.write(f"接続詞の検出率 (緩和) - Precision: {total_d_precision_relaxed:.2f}%, Recall: {total_d_recall_relaxed:.2f}%, F1: {total_d_f1_relaxed:.2f}%\n")
            out_file.write(f"相槌の脱落誤り数: {total_f_deletion_count}, 挿入誤り数: {total_f_insertion_count}\n")
            out_file.write(f"接続詞の脱落誤り数: {total_d_deletion_count}, 挿入誤り数: {total_d_insertion_count}\n")
            out_file.write(f"相槌の脱落誤り数 (緩和): {total_f_deletion_count_relaxed}, 挿入誤り数 (緩和): {total_f_insertion_count_relaxed}\n")
            out_file.write(f"接続詞の脱落誤り数 (緩和): {total_d_deletion_count_relaxed}, 挿入誤り数 (緩和): {total_d_insertion_count_relaxed}\n")
...
text1_path = "HYP/b_c/text"
text2_path = "REF/b_c/text"
output_path = "b_c_detection_rate.txt"
