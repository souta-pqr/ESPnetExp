    ...
    with open(output_path, 'w', encoding='utf-8') as out_file:
        ...
        with open("detection_rate_detail.txt", 'w', encoding='utf-8') as detail_file:
            ...
            for i in range(1, len(sentences1), 2):
                ...
                out_file.write(f"フィラーの検出率 - Precision: {filler_precision:.2f}%, Recall: {filler_recall:.2f}%\n")
                out_file.write(f"言い直しの検出率 - Precision: {disfluency_precision:.2f}%, Recall: {disfluency_recall:.2f}%\n")
                out_file.write(f"フィラーの検出率 (緩和) - Precision: {filler_precision_relaxed:.2f}%, Recall: {filler_recall_relaxed:.2f}%\n")
                out_file.write(f"言い直しの検出率 (緩和) - Precision: {disfluency_precision_relaxed:.2f}%, Recall: {disfluency_recall_relaxed:.2f}%\n")
            ...
            # テキスト全体でのフィラーと言い直しのPrecision、Recallを計算
            ...
            out_file.write(f"フィラーの検出率 - Precision: {total_filler_precision:.2f}%, Recall: {total_filler_recall:.2f}%\n")
            out_file.write(f"言い直しの検出率 - Precision: {total_disfluency_precision:.2f}%, Recall: {total_disfluency_recall:.2f}%\n")
            out_file.write(f"フィラーの検出率 (緩和) - Precision: {total_filler_precision_relaxed:.2f}%, Recall: {total_filler_recall_relaxed:.2f}%\n")
            out_file.write(f"言い直しの検出率 (緩和) - Precision: {total_disfluency_precision_relaxed:.2f}%, Recall: {total_disfluency_recall_relaxed:.2f}%\n")
